...
class Connection(object):
    ...
    def __init__(self, *args, **kwargs):
        self._args = args
        self._kwargs = kwargs
        _conf = kwargs.pop('KWEST_CONF')
        self._host = kwargs.get('host')
        self._port = kwargs.get('port')
        with open(_conf, 'r') as configFile:
            self._config = yaml.load(configFile, yaml.SafeLoader)
        self._token = self.get_kwest_token(self._config)
    ...
    def cursor(self):
        ...
        return Cursor(config=self._config, kwest_token=self._token, *self._args, **self._kwargs)

    def rollback(self):
        raise NotSupportedError("Presto does not have transactions")  # pragma: no cover

    def get_kwest_token(self, conf):
        session = Session()
        url = urlparse.urlunparse((
            "https",
            '{}:{}'.format(self._host, self._port), '/v1/login', None, None, None))

        try:
            response = session.post(url, 'user=' + conf['User'] + '&group=' + conf['Group'] + '&credential=', verify=False, cert=(conf['ClientCertificates'], conf['ClientKey']), headers={'Content-Type': 'application/x-www-form-urlencoded', 'x-kwest-authn': 'MT'},)
        except Exception as err:
            raise RuntimeError(f'Kwest login failed: {err}')

        if response and response.status_code == 200:
            return response.text

        raise RuntimeError(f'Kwest login failed: {response}')
...
class Cursor(common.DBAPICursor):
    ...
    """
    def __init__(self, config, kwest_token, host, port='8080', username=None, principle_username=None, catalog='hive',
        ...
        """
        :param config: kwest config
        :param kwest_token: security token for accessing the Kwest service
    ...
    def execute(self, operation, parameters=None):
        ...
        headers = {'Content-Type': 'text/plain',
                   'x-kwest-user': self._config.get('User'),
                   'x-kwest-catalog': self._catalog,
                   'x-kwest-schema': self._schema,
                   'x-kwest-token': self._kwest_token}
        ...
        url = urlparse.urlunparse((
            "https",
            '{}:{}'.format(self._host, self._port), '/v1/query', None, None, None))
        ...
        response = self._requests_session.post(
            url, data=sql.encode('utf-8'), headers=headers, verify=False, **self._requests_kwargs)
    ...
    def poll(self):
        ...
        if self._nextUri is None:
            assert self._state == self._STATE_FINISHED, "Should be finished if nextUri is None"
            return None
        response = self._requests_session.get(self._nextUri, verify=False, **self._requests_kwargs)
        self._process_response(response)
        return response.json()

    def _fetch_more(self):
        """Fetch the next URI and update state"""
        headers={'x-kwest-token': self._kwest_token}
        self._process_response(self._requests_session.get(self._nextUri, headers=headers, verify=False, **self._requests_kwargs))
